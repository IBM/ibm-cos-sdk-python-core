...
import ibm_botocore
from ibm_botocore.compress import COMPRESSION_MAPPING, maybe_compress_request
...
@mock.patch.object(ibm_botocore.compress, 'GzipFile', StaticGzipFile)
@mock.patch.object(ibm_botocore.compress, 'gzip_compress', static_compress)
@pytest.mark.skipif(
...
@mock.patch.object(ibm_botocore.compress, 'gzip_compress', static_compress)
@mock.patch.object(ibm_botocore.compress, 'COMPRESSION_MAPPING', MOCK_COMPRESSION)
@pytest.mark.skipif(
    sys.version_info < (3, 8), reason='requires python3.8 or higher'
)
