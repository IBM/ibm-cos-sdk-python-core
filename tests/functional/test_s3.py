...
from ibm_botocore.compat import get_md5, parse_qs, urlsplit
...
class TestAccesspointArn(BaseS3ClientConfigurationTest):
    ...
    def test_accesspoint_arn_with_s3_accelerate(self):
        ...
        with self.assertRaises(
            ibm_botocore.exceptions.UnsupportedS3AccesspointConfigurationError
        ):
            self.client.list_objects(Bucket=accesspoint_arn)
    ...
    @FreezeTime(ibm_botocore.auth.datetime, date=DATE)
...
class TestS3PutObject(BaseS3OperationTest):
    def test_500_error_with_non_xml_body(self):
        ...
        # the response. IBM_Botocore will try to parse out the error body as xml,
...
class TestGeneratePresigned(BaseS3OperationTest):
    ...
    #     config = Config(
    #         signature_version=ibm_botocore.UNSIGNED, s3={"use_accelerate_endpoint": True}
    #     )
...
def _addressing_for_presigned_url_test_cases():
    ...
    # https://github.com/ibm_boto/ibm_botocore/issues/2798
    yield dict(
        region="us-west-1",
        bucket="foo.bar.biz",
        key="key",
