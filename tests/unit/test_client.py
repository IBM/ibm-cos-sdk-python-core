...
from ibm_botocore.endpoint import DEFAULT_TIMEOUT
...
class TestAutoGeneratedClient(unittest.TestCase):
    def setUp(self):
        ...
        self.endpoint_creator_patch = mock.patch(
            'ibm_botocore.args.EndpointCreator')
    ...
    def create_client_creator(self, endpoint_creator=None, event_emitter=None,
        ...
        if retry_handler_factory is None:
            retry_handler_factory = ibm_botocore.retryhandler
        if retry_config_translator is None:
            retry_config_translator = ibm_botocore.translate
    ...
    #     creator = self.create_client_creator()
    #     with self.assertLogs('ibm_botocore.client', level='WARNING') as log:
    ...
    def test_signing_region_does_not_change_client_region(self):
        with mock.patch('ibm_botocore.args.RequestSigner') as mock_signer:
    ...
    @mock.patch('ibm_botocore.args.RequestSigner')
    ...
    def test_client_signature_override_arg(self, request_signer):
        creator = self.create_client_creator()
        config = ibm_botocore.config.Config(signature_version='foo')
        creator.create_client(
            'myservice', 'us-west-2', credentials=self.credentials,
    ...
    def test_client_custom_user_agent_in_request(self):
        creator = self.create_client_creator()
        config = ibm_botocore.config.Config(user_agent='baz')
        service_client = creator.create_client(
            'myservice', 'us-west-2', client_config=config)
    ...
    def test_client_custom_user_agent_extra_in_request(self):
        creator = self.create_client_creator()
        config = ibm_botocore.config.Config(user_agent_extra='extrastuff')
        service_client = creator.create_client(
            'myservice', 'us-west-2', client_config=config)
        service_client.test_operation(Foo='one')
    ...
    @mock.patch('ibm_botocore.args.RequestSigner.sign')
    ...
    def test_client_config_trumps_scoped_config(self):
        ...
        client_config = ibm_botocore.config.Config(parameter_validation=False)
    ...
    def test_client_with_custom_both_timeout(self):
        self.create_client_creator().create_client(
            'myservice', 'us-west-2',
            client_config=ibm_botocore.config.Config(connect_timeout=123, read_timeout=234))
    ...
    def test_client_with_custom_connect_timeout(self):
        self.create_client_creator().create_client(
            'myservice', 'us-west-2',
            client_config=ibm_botocore.config.Config(connect_timeout=123))
        call_kwargs = self.endpoint_creator.create_endpoint.call_args[1]
    ...
    def test_client_with_custom_read_timeout(self):
        self.create_client_creator().create_client(
            'myservice', 'us-west-2',
            client_config=ibm_botocore.config.Config(read_timeout=234))
        call_kwargs = self.endpoint_creator.create_endpoint.call_args[1]
    ...
    def test_client_with_custom_proxy_config(self):
        ...
        self.create_client_creator().create_client(
            'myservice', 'us-west-2',
            client_config=ibm_botocore.config.Config(proxies_config=proxies_config)
        )
        call_kwargs = self.endpoint_creator.create_endpoint.call_args[1]
    ...
    def test_can_override_max_attempts(self):
        retry_handler_factory = mock.Mock(ibm_botocore.retryhandler)
        creator = self.create_client_creator(
            retry_handler_factory=retry_handler_factory)
        creator.create_client(
            'myservice', 'us-west-2',
            client_config=ibm_botocore.config.Config(retries={'max_attempts': 9,
                                                          'mode': 'legacy'}))
    ...
    def test_can_register_standard_retry_mode(self):
        with mock.patch('ibm_botocore.client.standard') as standard:
            creator = self.create_client_creator()
            creator.create_client(
                'myservice', 'us-west-2',
                client_config=ibm_botocore.config.Config(
    ...
    def test_client_s3_addressing_style_with_config(self):
        ...
        my_client = creator.create_client(
            'myservice', 'us-west-2',
            client_config=ibm_botocore.config.Config(s3={'addressing_style': 'auto'})
        )
        self.assertEqual(
    ...
    def test_client_s3_addressing_style_with_incorrect_style(self):
        with self.assertRaises(InvalidS3AddressingStyleError):
            ibm_botocore.config.Config(s3={'addressing_style': 'foo'})
    ...
    def test_client_operation_hostname_binding_configuration(self):
        config = ibm_botocore.config.Config(inject_host_prefix=False)
    ...
    def test_client_internal_credential_shim(self):
        ...
        our upload/download operations in ibm_s3transfer with the CRT.
        """
        creator = self.create_client_creator()
        service_client = creator.create_client(
            'myservice', 'us-west-2', credentials=self.credentials
...
class TestConfig(unittest.TestCase):
    def test_can_use_args_to_construct(self):
        config = ibm_botocore.config.Config(*ibm_botocore.config.Config.OPTION_DEFAULTS.values())
        for option, default_value in ibm_botocore.config.Config.OPTION_DEFAULTS.items():
    ...
    def test_can_use_kwargs_to_construct(self):
        config = ibm_botocore.config.Config(**ibm_botocore.config.Config.OPTION_DEFAULTS)
        for option, default_value in ibm_botocore.config.Config.OPTION_DEFAULTS.items():
            self.assertTrue(hasattr(config, option))
            self.assertEqual(getattr(config, option), default_value)

    def test_can_use_mix_of_args_and_kwargs(self):
        config = ibm_botocore.config.Config('us-east-1', read_timeout=50)
    ...
    def test_merge_overrides_values(self):
        config = ibm_botocore.config.Config(region_name='us-east-1')
        other_config = ibm_botocore.config.Config(region_name='us-west-2')
        new_config = config.merge(other_config)
        self.assertEqual(new_config.region_name, 'us-west-2')

    def test_merge_overrides_values_even_when_using_default(self):
        config = ibm_botocore.config.Config(region_name='us-east-1')
        other_config = ibm_botocore.config.Config(region_name=None)
        new_config = config.merge(other_config)
        self.assertEqual(new_config.region_name, None)

    def test_merge_overrides_values_even_when_using_default_timeout(self):
        config = ibm_botocore.config.Config(read_timeout=30)
        other_config = ibm_botocore.config.Config(read_timeout=DEFAULT_TIMEOUT)
        new_config = config.merge(other_config)
    ...
    def test_can_set_retry_max_attempts(self):
        config = ibm_botocore.config.Config(retries={'max_attempts': 15})
...
class TestClientEndpointBridge(unittest.TestCase):
    ...
    def test_disable_dualstack_explicitly(self):
        ...
        config = ibm_botocore.config.Config(s3={'use_dualstack_endpoint': False})
    ...
    def test_use_dualstack_endpoint(self):
        config = ibm_botocore.config.Config(use_dualstack_endpoint=True)
    ...
    #     config = ibm_botocore.config.Config(use_fips_endpoint=True)
    #     bridge = ClientEndpointBridge(self.resolver, client_config=config)
    #     bridge.resolve('ec2', 'us-west-2')
    #     self.resolver.construct_endpoint.assert_called_with(
    #         'ec2',
    ...
    def test_modeled_endpoint_variants_client_config_trumps_scoped_config(self):
        ...
        config = ibm_botocore.config.Config(use_dualstack_endpoint=False,
                                        # IBM Unsupported
                                        # use_fips_endpoint=False
                                        )
        bridge = ClientEndpointBridge(self.resolver, scoped_config,
