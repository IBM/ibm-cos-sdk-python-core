...
To modify the User-Agent header sent by ibm_botocore, use one of these
configuration options:
* The ``AWS_SDK_UA_APP_ID`` environment variable.
* The ``sdk_ua_app_id`` setting in the shared AWS config file.
* The ``user_agent_appid`` field in the :py:class:`ibm_botocore.config.Config`.
* The ``user_agent_extra`` field in the :py:class:`ibm_botocore.config.Config`.
...
from ibm_botocore import __version__ as ibm_botocore_version
...
# The name by which ibm_botocore is identified in the User-Agent header. While most
# AWS SDKs follow a naming pattern of "aws-sdk-*", ibm_botocore and ibm_boto3 continue
...
try:
    from ibm_botocore.customizations.useragent import modify_components
except ImportError:
...
class UserAgentString:
    ...
    def set_session_config(
        ...
        :param user_agent_name: The user agent name configured in the
            :py:class:`ibm_botocore.session.Session` object. For backwards
        ...
        :param user_agent_version: The user agent version configured in the
            :py:class:`ibm_botocore.session.Session` object.
    ...
    def with_client_config(self, client_config):
        ...
        :type client_config: ibm_botocore.config.Config
    ...
    def _build_sdk_metadata(self):
        ...
        if (
            ...
            and (
                ...
                or self._session_user_agent_version != ibm_botocore_version
            )
        ):
            sdk_md.extend(
                [
                    ...
                    UserAgentComponent(
                        'md', _USERAGENT_SDK_NAME, ibm_botocore_version
                    ),
                ]
            )
        else:
            sdk_md.append(
                UserAgentComponent(_USERAGENT_SDK_NAME, ibm_botocore_version)
            )
    ...
    def _build_app_id(self):
        ...
        ``user_agent_appid`` field in :py:class:`ibm_botocore.config.Config` or
    ...
    def _build_extra(self):
        ...
        ibm_botocore's User-Agent header string are the ``user_agent_appid` field
        in :py:class:`ibm_botocore.config.Config`. The ``AWS_SDK_UA_APP_ID``
