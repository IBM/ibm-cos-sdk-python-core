...
def test_ibm_boto3_user_agent(patched_session):
    # emulate Boto3's behavior
    ibm_botocore_info = f'Botocore/{patched_session.user_agent_version}'
    if patched_session.user_agent_extra:
        patched_session.user_agent_extra += ' ' + ibm_botocore_info
    else:
        patched_session.user_agent_extra = ibm_botocore_info
    ...
    assert stub_client.captured_ua_string.startswith(
        f'Boto3/9.9.9 md/Botocore#{ibm_botocore_version} '
    )
...
def test_awscli_v1_user_agent(patched_session):
    ...
    patched_session.user_agent_extra = f'ibm_botocore/{ibm_botocore_version}'
    ...
    # above, followed by Botocore's version info as metadata ("md/...").
    assert stub_client.captured_ua_string.startswith(
        f'aws-cli/1.1.1 md/Botocore#{ibm_botocore_version} '
    )
    # The regular User-Agent header components for platform, language, ...
    # should also be present:
    assert ' ua/2.0 ' in stub_client.captured_ua_string
...
def test_awscli_v2_user_agent(patched_session):
    ...
    # above, followed by Botocore's version info as metadata ("md/...").
    assert stub_client.captured_ua_string.startswith(
        f'aws-cli/2.2.2 md/Botocore#{ibm_botocore_version} '
    )
...
def test_ibm_s3transfer_user_agent(patched_session):
    # emulate behavior from ibm_s3transfer ClientFactory
    cfg = Config(user_agent_extra='ibm_s3transfer/0.1.2 processpool')
    client = patched_session.create_client('s3', config=cfg)
    # ibm_s3transfer tests make assertions against the _modified_ `user_agent` field
    # in ``client.meta.config.user_agent``. See for example
    # ``tests.unit.test_processpool.TestClientFactory`` in ibm_s3transfer.
...
def test_chalice_user_agent(patched_session):
    ...
    with UACapHTTPStubber(client_s3) as stub_client:
        client_s3.list_buckets()
    assert stub_client.captured_ua_string.startswith(
        f'aws-chalice/0.1.2 md/Botocore#{ibm_botocore_version} '
    )
