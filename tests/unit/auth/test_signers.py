...
from ibm_botocore.compat import HTTPHeaders, parse_qs, urlsplit
...
class BaseTestWithFixedDate(unittest.TestCase):
    def setUp(self):
        ...
        self.datetime_patch = mock.patch('ibm_botocore.auth.datetime.datetime')
...
class TestSigV2(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.time_patcher = mock.patch.object(
            ibm_botocore.auth.time, 'gmtime', mock.Mock(wraps=time.gmtime)
        )
...
class TestS3SigV4Auth(BaseTestWithFixedDate):
    ...
    AuthClass = ibm_botocore.auth.S3SigV4Auth
    maxDiff = None
...
class TestSigV4(unittest.TestCase):
    ...
    def create_signer(self, service_name='myservice', region='us-west-2'):
        auth = ibm_botocore.auth.SigV4Auth(self.credentials, service_name, region)
        return auth
...
class TestSigV4Resign(BaseTestWithFixedDate):
    ...
    maxDiff = None
    AuthClass = ibm_botocore.auth.SigV4Auth
...
class TestSigV4Presign(BasePresignTest):
    ...
    maxDiff = None
    AuthClass = ibm_botocore.auth.SigV4QueryAuth
